...
pub fn decode(ins: u32) -> Result<Instruction, RiscVError> {
    ...
    Ok(match opcode {
        ...
        OpCode::ITYPE_LOAD => {
            let imm = ((ins & Bits::_31_TO_20) as i32) >> 20;
            Instruction::ItypeLoad(Itype { rd, rs1, imm, funct3 })
        },
        // imm [11:0] | rs1 [4:0] | funct3 [2:0] | rd [4:0] | opcode [6:0]
        OpCode::ITYPE_JUMP => {
            let imm = ((ins & Bits::_31_TO_20) as i32) >> 20;
            Instruction::ItypeJump(Itype { rd, rs1, imm, funct3 })
        },
        // imm [11:0] | rs1 [4:0] | funct3 [2:0] | rd [4:0] | opcode [6:0]
        OpCode::ITYPE_SYS => {
            let imm = ((ins & Bits::_31_TO_20) as i32) >> 20;
            Instruction::ItypeSys(Itype { rd, rs1, imm, funct3 })
        },
        // funct7 [6:0] | rs2 [4:0] | rs1 [4:0] | funct3 [2:0]  | rd [4:0] | opcode [6:0]
        OpCode::RTYPE => Instruction::Rtype(Rtype { rd, rs1, rs2, funct3, funct7 }),
        // imm [11:5] | rs2 [4:0] | rs1 [4:0] | funct3 [2:0] | imm [4:0] | opcode [6:0]
        OpCode::STYPE => {
            let imm =
                (((ins & Bits::_31_TO_25) as i32) >> 20) | (((ins & Bits::_11_TO_7) >> 7) as i32);
            Instruction::Stype(Stype { rs1, rs2, imm, funct3 })
        },
        // imm[12|10:5] | rs2 [4:0] | rs1 [4:0] | funct3 [2:0] | imm[4:1|11] | opcode [6:0]
        OpCode::BTYPE => {
            let imm = (((ins & Bits::_31) as i32) >> 19)
                | ((((ins & Bits::_7) << 4)
                    | ((ins & Bits::_30_TO_25) >> 20)
                    | ((ins & Bits::_11_TO_8) >> 7)) as i32);
            Instruction::Btype(Btype { rs1, rs2, imm, funct3 })
        },
        // imm[20|10:1|11|19:12] | rd[4:0] | opcode[6:0]
        OpCode::JTYPE => {
            let imm = (((ins & Bits::_31) as i32) >> 11)
                | (((ins & Bits::_19_TO_12)
                    | ((ins & Bits::_20) >> 9)
                    | ((ins & Bits::_30_TO_21) >> 20)) as i32);
...
mod decoder_test {
    ...
    fn err_test() {
        ...
        assert_eq!(
            ...
            Err(RiscVError::NotImplementedOpCode(0x7f))
        );
        assert_eq!(
            decoder::decode(err_ins2),
            Err(RiscVError::NotImplementedOpCode(0x0))
        );
    }
}
